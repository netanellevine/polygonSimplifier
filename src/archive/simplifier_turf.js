...
const new_file_name = `turf_${country_name}_t-${t}_l-${size_after}.geojson`;
...
function checkSimplified(data, str='') {
  const data_arr = [];
  var data_size = 0;
  var coords = coordAll(data);
  data_size = coords.length;
...
// simplifyShape(tolerance=1 ,highQuality=true);
